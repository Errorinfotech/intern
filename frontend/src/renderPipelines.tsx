    ...
    return (
        <div className="w-full flex justify-center items-center py-10 px-4">
            <div className="max-w-7xl mx-auto flex flex-col items-center w-full">
                {!isAIML && (
                    <div className="flex gap-4 mb-20 relative z-20">
                        <div 
                            role="button"
                            tabIndex={0}
                            onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleModeChange(3); }}
                            style={{ backgroundColor: mode === 3 ? '#bb734b' : 'transparent', borderColor: mode === 3 ? '#bb734b' : 'rgba(255,255,255,0.2)' }}
                            className="px-8 py-4 rounded-xl border-2 text-white font-bold transition-all duration-300 cursor-pointer hover:shadow-[0_0_20px_rgba(187,115,75,0.3)] min-w-[180px] text-center"
                        >
                            3 Month (Pro)
                        </div>
                        <div 
                            role="button"
                            tabIndex={0}
                            onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleModeChange(6); }}
                            style={{ backgroundColor: mode === 6 ? '#bb734b' : 'transparent', borderColor: mode === 6 ? '#bb734b' : 'rgba(255,255,255,0.2)' }}
                            className="px-8 py-4 rounded-xl border-2 text-white font-bold transition-all duration-300 cursor-pointer hover:shadow-[0_0_20px_rgba(187,115,75,0.3)] min-w-[220px] text-center"
                        >
                            6 Month (Enterprise)
                        </div>
                    </div>
